...
3. Thin `WallNotesLayer` composes renderers + windowing props only.
   - **Started 2026-06-18:** compact/ambient canvas rendering now lives in `src/components/wall/spatial/notes/renderers/WallCompactNoteRenderer.tsx`; full-detail image and web-bookmark branches now live in `WallImageNoteRenderer.tsx` and `WallBookmarkNoteRenderer.tsx`. `WallNotesLayer` still owns interaction wiring and the remaining full-detail note-kind rendering.
