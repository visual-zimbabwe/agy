...
For large wall snapshots, the handler writes in chunks and retries timed-out chunks at a smaller size before returning an error. This reduces `statement timeout` failures on larger accounts without changing the API contract.
