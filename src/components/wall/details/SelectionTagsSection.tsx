...
export const SelectionTagsSection = ({
  ...
  const hasSelection = selectedNoteIdsCount > 0 || Boolean(selectedNoteId);
    ...
    <div className={detailSectionCard}>
      ...
      <div className="mt-2 flex items-center gap-2">
        <input
          ...
          placeholder={hasSelection ? "add-tag" : "select note first"}
          disabled={!hasSelection || isTimeLocked}
          className={`flex-1 ${detailField}`}
        />
        <button type="button" onClick={onAddTag} disabled={!hasSelection || isTimeLocked} className={detailButton}>
          Add
        </button>
      </div>
