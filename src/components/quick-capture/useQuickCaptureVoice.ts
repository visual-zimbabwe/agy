...
type UseQuickCaptureVoiceOptions = {
  open: boolean;
  disabled?: boolean;
  onAppendText: (text: string) => void;
};

type UseQuickCaptureVoiceResult = {
...
export const useQuickCaptureVoice = ({
  open,
  disabled,
  onAppendText,
}: UseQuickCaptureVoiceOptions): UseQuickCaptureVoiceResult => {
  ...
  const flushInterim = useCallback(() => {
    ...
    onAppendText(pending);
    setInterimTranscript("");
    interimTranscriptRef.current = "";
  }, [onAppendText]);

  const startRecognition = useCallback(
    (sessionId: number, reason: "start" | "restart" | "resume") => {
  ...
  const createRecognition = useCallback(
    ...
    [clearRestartTimer, flushInterim, onAppendText, pushVoiceDebug, startRecognition, terminalErrors],
  );
  ...
  }, [clearRestartTimer, flushInterim, pushVoiceDebug]);
